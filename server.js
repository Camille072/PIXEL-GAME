...
io.on('connection', (socket) => {
  ...
  // PLAYER joins a room
  socket.on('join_room', ({ code, name, character, curso }, cb) => {
    ...
    if (Object.keys(room.players).length >= 100) return cb({ error: 'La sala está llena (máx. 100 jugadores).' });
    ...
    room.players[socket.id] = { name, character, curso: curso || '', score: 0, answers: [] };
    socket.join(code);
    socket.roomCode = code;
...
function sendNextQuestion(code) {
  ...
  if (room.currentQ >= QUESTIONS.length) {
    ...
    const leaderboard = Object.values(room.players)
      ...
      .map((p, i) => ({ rank: i + 1, name: p.name, character: p.character, curso: p.curso, score: p.score }));
